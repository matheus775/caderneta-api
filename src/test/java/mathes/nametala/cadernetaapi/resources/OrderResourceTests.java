...
public class OrderResourceTests {
	...
	void getOrders_noFilter_OK() throws Exception {
		...
		when(orderService.getOrders(null, null)).thenReturn(new PageImpl<OrderEntity>(new ArrayList<>()));
	...
	void getOrder_existentID_OK() throws Exception {
		...
		when(orderService.getOrder(2L)).thenReturn(expectedResult);

		ResultActions resultActions = mockMvc.perform(get("/orders/{id}", 2L)).andExpect(status().isOk());
	...
	void getOrder_nonexistentID_NOT_FOUND() throws Exception {
		...
		mockMvc.perform(get("/orders/{id}", 0L)).andExpect(status().isNotFound());
	...
	public void postOrder_correctData_OK() throws Exception {
		...
		order.setId(null);
		when(orderService.newOrder(order)).thenReturn(expectedResult);

		ResultActions resultActions = mockMvc
				.perform(post("/orders").sessionAttr(TOKEN_ATTR_NAME, csrfToken)
						.param(csrfToken.getParameterName(), csrfToken.getToken())
						.contentType(MediaType.APPLICATION_JSON).content(order.toString()))
	...
	public void postOrder_negativeTotal_BAD_REQUEST() throws Exception {
		...
		mockMvc.perform(post("/orders").sessionAttr(TOKEN_ATTR_NAME, csrfToken)
				...
				.content(order.toString())).andExpect(status().isBadRequest());
	...
	void delOrder_nonexistentID_NOT_FOUND() throws Exception {
		...
		mockMvc.perform(delete("/orders/{id}", 0L).sessionAttr(TOKEN_ATTR_NAME, csrfToken)
	...
	public void putOrder_correctData_OK() throws Exception {
		...
		when(orderService.uptdOrder(expectedResult, 2L)).thenReturn(expectedResult);

		ResultActions resultActions = mockMvc
				.perform(put("/orders/{id}", 2L).sessionAttr(TOKEN_ATTR_NAME, csrfToken)
						.param(csrfToken.getParameterName(), csrfToken.getToken())
						.contentType(MediaType.APPLICATION_JSON).content(expectedResult.toString()))
	...
	public void putOrder_nonExistentId_BAD_REQUEST() throws Exception {
		...
		when(orderService.uptdOrder(order, 0L)).thenThrow(NoSuchElementException.class);

		mockMvc.perform(put("/orders/{id}", 0L).sessionAttr(TOKEN_ATTR_NAME, csrfToken)
				.param(csrfToken.getParameterName(), csrfToken.getToken()).contentType(MediaType.APPLICATION_JSON)
				.content(order.toString())).andExpect(status().isNotFound());
	...
	public void putOrder_negativeTotal_BAD_REQUEST() throws Exception {
		...
		mockMvc.perform(put("/orders/{id}", 2L).sessionAttr(TOKEN_ATTR_NAME, csrfToken)
				.param(csrfToken.getParameterName(), csrfToken.getToken()).contentType(MediaType.APPLICATION_JSON)
				.content(order.toString())).andExpect(status().isBadRequest());
